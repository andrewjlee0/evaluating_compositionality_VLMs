...
starting_num = 15
problem_sequence = list(range(starting_num,24))
data = pd.read_csv(f'results/{starting_num-1}_limited_results.csv').drop(columns='Unnamed: 0')
print(data.columns)
...
with open('results/randomized_trial_sequence_dict.pkl', 'wb') as file:
    ...
    for trial, file_path in enumerate(randomized_trial_sequence, 1):
        ...
        while True:
            ...
            try:
                ...
                # Save convo history
                with open(f'results/{problem}_convo_history.pkl', 'wb') as file:
        ...
        save_path = f'results/{problem}_limited_results.csv'
